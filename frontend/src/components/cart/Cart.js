...
function Cart() {
    ...
    useEffect (() => {
        if (fixEffect.current) {
            axios.post("http://localhost:8000/cart/getall",
                        ...
                        return (
                            <div key={rndom()} name={JSON.stringify(obj)} className="cars cars1">
                                <div className="card card1" >
                                    ...
                                    <p className="sp spName">{obj.name.substring(0,20)+"..."}</p>
                                    <p className="sp spDescription">{obj.description.substring(0,100)+"..."}</p>
                                    <p className="sp spPrice">Rs {obj.price}</p>
                                    <p className="sp spQuantity">Quantity &nbsp;&nbsp;
                                    <input name={JSON.stringify(obj)} className='quantityInput'   value={1}  onChange={(e) => quantityChange(e)}></input></p>
